...
There are various so-called _loss functions_ these days, which essentially present you the [difference between true target and prediction](https://github.com/mobiletest2016/machine-learning-articles/blob/master/articles/about-loss-and-loss-functions). The Kullback-Leibler divergence (or KL Divergence for short) is one of these. Seeing it in the Keras docs spawned a lot of questions. What is KL divergence? How does it work as a loss function? In what kind of machine learning (or deep learning) problems can it be used? And how can I implement it?
...
Intuitively, it's the _expected value of the probability of data in some distribution_. In plain English, it's something like this (assuming that $log\_2$ is used): "the minimum number of bits it would take us to encode our information" (Count Bayesie, 2017).
...
We'll therefore slightly adapt a [ConvNet created in another blog post](https://github.com/mobiletest2016/machine-learning-articles/blob/master/articles/how-to-create-a-cnn-classifier-with-keras.md) to use KL divergence. This way, you don't have to immerse yourself in an entirely new model (assuming that you've read the linked post) yet can see how KL divergence can be used with Keras.
